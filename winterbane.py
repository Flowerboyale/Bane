...
async def main(sprinter):
    ...
    while True:
        ...
        await p3.teleport(p1pos)
        await p3.send_key(Keycode.W, 0.1)
        await asyncio.sleep(0.3)

        combat_handlers = []
        print("Initiating combat")
        combat_handlers.append(SprintyCombat(p1, CombatConfigProvider('bane_configs/p1spellconfig.txt', cast_time=1.5)))
        combat_handlers.append(SprintyCombat(p2, CombatConfigProvider('bane_configs/p2spellconfig.txt', cast_time=1.5)))
        combat_handlers.append(SprintyCombat(p3, CombatConfigProvider('bane_configs/p3spellconfig.txt', cast_time=1.5)))
        ...
        await asyncio.sleep(1)
        await p3.send_key(Keycode.A, 0.1)
        await asyncio.sleep(1)
        ...
        await p4.goto(-2650.177978515625, 71.61460876464844)
        await asyncio.sleep(3)
        await p3.goto(3050.296630859375, 17.513824462890625)
        await asyncio.sleep(3)
        await p4.goto(3050.296630859375, 17.513824462890625)
        await asyncio.sleep(3)
        
        for p in clients:
          await p.use_potion_if_needed(health_percent=10, mana_percent=7)
          await asyncio.sleep(1)
